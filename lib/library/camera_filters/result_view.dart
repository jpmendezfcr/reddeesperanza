                    ...
                    button(
                      ...
                      () async {
                        if (widget.color == Colors.transparent) {
                          widget.onDone?.call(widget.fileURL, _videoPlayerController?.value.duration.inSeconds.toDouble() ?? 0);
                        } else {
                          if (widget.isVideo) {
                            List<TapiocaBall> tapiocaBalls = [TapiocaBall.filterFromColor(Color(widget.color.withValues(alpha: 0.1).value))];
  ...
  Widget button(String title, Function() onTap) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: cWhite, borderRadius: BorderRadius.circular(100), boxShadow: [BoxShadow(color: cLightBg)]),
        child: Text(
          title.tr,
