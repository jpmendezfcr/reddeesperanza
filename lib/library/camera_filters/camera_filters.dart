...
class CameraScreenPlugin extends StatefulWidget {
  ...
  CameraScreenPlugin({Key? key, this.onDone, this.onVideoDone, this.filters, this.profileIconWidget, this.applyFilters = true, this.gradientColors, this.sendButtonWidget, this.filterColor}) : super(key: key);
...
class _CameraScreenState extends State<CameraScreenPlugin> with WidgetsBindingObserver {
                      ...
                      return ValueListenableBuilder(
                          ...
                          builder: (context, value, child) {
                            return ColorFiltered(
                              colorFilter: ColorFilter.mode(widget.filterColor == null ? _filterColor.value : widget.filterColor!.value, BlendMode.softLight),
                              child: _controller == null || !_controller!.value.isInitialized ? Container() : Transform.scale(scale: 1 / ((_controller!.value.aspectRatio) * MediaQuery.of(context).size.aspectRatio), alignment: Alignment.topCenter, child: CameraPreview(_controller!)),
                            );
                          });
                    } else {
            ...
            () {
              Get.bottomSheet(StoryImagePicker(
                ...
                onGetPath: (path, isVideo) {
                  Navigate.to(ResultView(isVideo: isVideo, fileURL: path, onDone: widget.onVideoDone, color: Colors.transparent))?.then((value) {
          ...
          ValueListenableBuilder(
              ...
              builder: (context, value, Widget? c) {
                return time.value == ""
                    ? Container()
                    : Container(
                        decoration: const ShapeDecoration(shape: SmoothRectangleBorder(borderRadius: SmoothBorderRadius.all(SmoothRadius(cornerRadius: 12, cornerSmoothing: cornerSmoothing))), color: cBlack),
                        padding: const EdgeInsets.only(right: 10, left: 10, top: 5, bottom: 3),
                        child: Text(
                          formatHHMMSS(int.tryParse(time.value) ?? 0),
    ...
    return FilterSelector(
      ...
      onLongPress: () async {
        await _controller?.startVideoRecording(
          onAvailable: (image) {
            String timeValue = time.value;
            int seconds = int.parse(timeValue) ~/ 100;
  ...
  Future stopRecording() async {
    ...
    try {
      ...
      print('Error While Camera $e');
    }

    log('Camera Message $file');
    flashCheck();
    var color = widget.filterColor == null ? _filterColor.value : widget.filterColor!.value;
    Navigate.to(ResultView(isVideo: true, fileURL: file?.path ?? '', onDone: widget.onVideoDone, color: color))?.then((value) {
      if (flashCount.value == 1) {
        _controller?.setFlashMode(FlashMode.torch);
      }
    });
