...
class CreateRoomScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Column(
        children: [
          TopBarForInView(title: room != null ? LKeys.roomSettings : LKeys.createNewRoom),
          Expanded(
                ...
                child: Column(
                  children: [
                    ...
                    GetBuilder<CreateRoomController>(
                        ...
                        builder: (controller) {
                          return CreateRoomHeading(
                            ...
                            bracketText: "(${controller.textDescriptionController.text.length}/${Limits.roomDescCount})",
                          );
                        }),
                    MyTextField(
                    ...
                    GetBuilder<CreateRoomController>(
                      ...
                      builder: (controller) {
                        return SettingButtonWithSwitch(title: LKeys.makeRoomPublic, desc: LKeys.makeRoomPublicDesc, isOn: controller.visibleToPublic, onChange: controller.toggleMakeRoomPublic);
                      },
                    ),
                    GetBuilder<CreateRoomController>(
                      init: controller,
                      ...
                      builder: (controller) {
                        return SettingButtonWithSwitch(title: LKeys.joinAfterRequest, desc: LKeys.joinAfterRequestDesc, isOn: controller.joinAfterRequest, onChange: controller.toggleJoinAfterReq);
                      },
                    ),
                    const SizedBox(height: 50),
                    SafeArea(
                      top: false,
                      child: CommonButton(text: room == null ? LKeys.createRoom : LKeys.update, onTap: controller.onSubmit),
                    ),
...
class CreateRoomHeading extends StatelessWidget {
  ...
  const CreateRoomHeading({Key? key, required this.title, this.bracketText}) : super(key: key);
...
class MyTextField extends StatelessWidget {
  ...
  const MyTextField({Key? key, required this.controller, required this.placeHolder, this.isEditor = false, this.color, this.limit, this.onChange, this.obscureText = false}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Container(
      height: isEditor ? 130 : 40,
      decoration: BoxDecoration(
        color: color ?? cLightBg,
        borderRadius: SmoothBorderRadius(cornerRadius: 8, cornerSmoothing: cornerSmoothing),
      ),
      ...
      child: TextField(
        ...
        decoration: InputDecoration(hintText: placeHolder.tr, hintStyle: MyTextStyle.gilroyRegular(color: cLightText.withValues(alpha: 0.6)), border: InputBorder.none, counterText: '', isDense: true, contentPadding: const EdgeInsets.all(0)),
