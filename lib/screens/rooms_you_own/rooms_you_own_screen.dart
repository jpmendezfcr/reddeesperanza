...
class MyRoomCard extends StatelessWidget {
  ...
  const MyRoomCard({Key? key, required this.room, required this.onBack}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () {
        Navigate.to(ChattingView(
          room: room,
        ))?.then((value) {
          onBack();
        });
      ...
      child: Column(
        children: [
          Container(
            ...
            child: Row(
              children: [
                ClipOval(child: MyCachedImage(imageUrl: room.photo, width: 60, height: 60)),
