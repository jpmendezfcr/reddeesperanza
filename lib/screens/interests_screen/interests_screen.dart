              ...
              Expanded(
                child: Center(
                  child: SingleChildScrollView(
                    child: GetBuilder<InterestsController>(builder: (controller) {
                      return Wrap(
...
class InterestTag extends StatelessWidget {
  ...
  const InterestTag({Key? key, required this.interest, required this.isContain, required this.onTap}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: ShapeDecoration(shape: const SmoothRectangleBorder(borderRadius: SmoothBorderRadius.all(SmoothRadius(cornerRadius: 23, cornerSmoothing: cornerSmoothing))), color: isContain ? cPrimary : cLightBg, shadows: isContain ? kMyBoxShadow : []),
        padding: const EdgeInsets.symmetric(vertical: 13, horizontal: 25),
        child: Text(
          interest.toUpperCase(),
          style: MyTextStyle.gilroyBold(color: isContain ? cBlack : cLightText, size: 14),
        ),
      ),
    );
  }
