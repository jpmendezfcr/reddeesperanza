...
class StoryScreen extends StatelessWidget {
  ...
  const StoryScreen({Key? key, required this.users, required this.index}) : super(key: key);
                        ...
                        Padding(
                          ...
                          child: GestureDetector(
                            onTap: () {
                              ...
                              Navigate.to(ProfileScreen(userId: user?.id ?? 0))?.then((value) {
                                    ...
                                    children: [
                                      Row(
                                        children: [
                                          ...
                                          const Spacer(),
                                          Text(
                                            item.story?.date.timeAgoShort() ?? 'now',
                                            style: MyTextStyle.gilroyLight(color: cWhite, size: 14),
                                          ),
                                          const SizedBox(width: 3),
                                          (user?.id == SessionManager.shared.getUserID())
                                              ? Menu(
                                                  items: [
                                                    PopupMenuItem(
                                                      ...
                                                      onTap: () {
                                                        controller.storyController.pause();
                                                        if (item.story != null) {
                                                          controller.deleteStory(item.story!);
                                                        }
                                                      },
                                                      child: Text(
                                                        LKeys.delete.tr,
                                      ...
                                      Text(
                                        ...
                                        style: MyTextStyle.gilroyLight(color: cWhite, size: 15),
                                      ),
                                    ],
                                  ),
                                ),
