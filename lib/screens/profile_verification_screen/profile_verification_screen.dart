...
class ProfileVerificationScreen extends StatelessWidget {
                  ...
                  builder: (controller) {
                    return Column(
                      children: [
                        ...
                        controller.selectedMethod.method == VerificationMethod.document ? documentVerificationView(controller) : subscriptionView(controller),
                        const SizedBox(height: 40),
                        CommonButton(
                          text: (controller.selectedMethod.method == VerificationMethod.document) ? LKeys.submit : LKeys.subscribe,
              ...
              itemBuilder: (context, index) {
                ...
                return GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(border: Border.all(color: controller.selectedPackage == package ? cPrimary : cLightText.withValues(alpha: 0.3), width: 2), borderRadius: SmoothBorderRadius(cornerRadius: 12)),
                    child: Row(
                      children: [
                        Text(
                        ...
                        Spacer(),
                        Text(package.storeProduct.priceString, style: MyTextStyle.gilroyBold(size: 18, color: cDarkText)),
                      ],
                    ),
                  ),
                );
  ...
  Widget segmentController(ProfileVerificationController controller) {
    return CupertinoSlidingSegmentedControl(
      children: {controller.methods.first: buildSegment(LKeys.method1, 0, controller), controller.methods.last: buildSegment(LKeys.method2, 1, controller)},
  ...
  Widget buildSegment(String text, int index, ProfileVerificationController controller) {
    return Container(
      ...
      child: Text(
        text.tr.toUpperCase(),
        style: MyTextStyle.gilroySemiBold(size: 13, color: controller.selectedMethod == controller.methods[index] ? cBlack : cLightText).copyWith(letterSpacing: 2),
      ),
    );
  }
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          MyTextField(controller: controller.fullNameController, placeHolder: SessionManager.shared.getUser()?.fullName ?? ''),
          const SizedBox(height: 10),
  ...
  Widget dropDown(ProfileVerificationController controller) {
    return Container(
      ...
      child: DropdownButton<DocumentType>(
        ...
        items: controller.types.map<DropdownMenuItem<DocumentType>>((DocumentType value) {
