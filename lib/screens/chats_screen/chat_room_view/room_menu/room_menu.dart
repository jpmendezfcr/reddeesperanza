...
class RoomMenu extends StatelessWidget {
  ...
  void shareRoom(Room? room) {
    BranchUniversalObject buo = BranchUniversalObject(canonicalIdentifier: 'flutter/branch', title: room?.title ?? '', imageUrl: room?.photo?.addBaseURL() ?? '', contentDescription: room?.desc ?? '', publiclyIndex: true, locallyIndex: true);
    ...
    if (GetPlatform.isIOS) {
      if (buo.imageUrl != '') {
        ...
        rootBundle.load(MyImages.appIcon).then((data) {
          FlutterBranchSdk.shareWithLPLinkMetadata(buo: buo, linkProperties: lp, icon: data.buffer.asUint8List(), title: room?.title ?? '');
        });
      }
    } else {
