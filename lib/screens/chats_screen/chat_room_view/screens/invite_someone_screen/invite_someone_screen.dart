              ...
              children: [
                TopBarForInView(
                  ...
                  verticalChild: Container(
                    ...
                    decoration: ShapeDecoration(shape: const SmoothRectangleBorder(borderRadius: SmoothBorderRadius.all(SmoothRadius(cornerRadius: 5, cornerSmoothing: cornerSmoothing))), color: cPrimary.withValues(alpha: 0.1)),
                    child: Text(
                      (room?.title ?? '').toUpperCase(),
                      style: MyTextStyle.gilroySemiBold(color: cPrimary, size: 12).copyWith(letterSpacing: 1),
                      maxLines: 1,
...
class RoomMemberCard extends StatelessWidget {
  ...
  const RoomMemberCard({Key? key, required this.type, required this.user, this.onTapInvitation, this.onRequestAccept, this.onRequestReject, this.onAdminRemove, this.onMakeAdmin, this.onRemoveFromRoom}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        GestureDetector(
          onTap: () {
            Navigate.to(ProfileScreen(userId: user.id ?? 0));
          },
          child: Row(
            children: [
              ClipSmoothRect(
    ...
    if (type == RoomMemberType.user) {
      return GestureDetector(
        ...
        child: Container(
          ...
          decoration: ShapeDecoration(color: cPrimary.withValues(alpha: 0.15), shape: const SmoothRectangleBorder(borderRadius: SmoothBorderRadius.all(SmoothRadius(cornerRadius: 5, cornerSmoothing: cornerSmoothing)))),
          child: Text(
            LKeys.invite.tr.toUpperCase(),
            style: MyTextStyle.gilroySemiBold(size: 11).copyWith(letterSpacing: 1),
          ),
        ),
      );
      ...
      return GestureDetector(
        ...
        child: Container(
          padding: const EdgeInsets.symmetric(vertical: 7, horizontal: 18),
          decoration: ShapeDecoration(color: cRed.withValues(alpha: 0.15), shape: const SmoothRectangleBorder(borderRadius: SmoothBorderRadius.all(SmoothRadius(cornerRadius: 5, cornerSmoothing: cornerSmoothing)))),
          child: Text(
      ...
      return Row(
        children: [
          CircleIcon(color: cGreen, iconData: Icons.check_rounded, onTap: onRequestAccept),
      ...
      return Menu(
        items: [PopupMenuItem(onTap: onMakeAdmin, child: LKeys.makeAdmin.toTextTR(MyTextStyle.gilroyMedium())), PopupMenuItem(onTap: onRemoveFromRoom, child: LKeys.removeFromRoom.toTextTR(MyTextStyle.gilroyMedium()))],
        color: cLightText,
      );
    } else if (type == RoomMemberType.owner) {
      return Container(
        padding: const EdgeInsets.symmetric(vertical: 7, horizontal: 18),
        decoration: ShapeDecoration(shape: const SmoothRectangleBorder(borderRadius: SmoothBorderRadius.all(SmoothRadius(cornerRadius: 5, cornerSmoothing: cornerSmoothing))), color: cGreen.withValues(alpha: 0.15)),
        child: Text(
          LKeys.owner.tr.toUpperCase(),
          style: MyTextStyle.gilroySemiBold(size: 11).copyWith(letterSpacing: 1, color: cGreen),
        ),
      );
    }
    return Container();
