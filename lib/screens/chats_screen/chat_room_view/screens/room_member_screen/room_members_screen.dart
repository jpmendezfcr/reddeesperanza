...
class RoomMembersScreen extends StatelessWidget {
  ...
  const RoomMembersScreen({Key? key, required this.chattingController}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    RoomMembersController controller = RoomMembersController(chattingController);
    return Scaffold(
      body: GetBuilder(
          init: controller,
          builder: (controller) {
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return RoomMemberCard(
                                    ...
                                    onRemoveFromRoom: () {
                                      controller.removeUserFromRoom(member);
                                    },
                                  );
                                },
                              ),
  ...
  Widget segmentController(RoomMembersController controller) {
    return CupertinoSlidingSegmentedControl(
      children: {0: buildSegment(LKeys.allMembers, 0, controller), 1: buildSegment(LKeys.admins, 1, controller)},
  ...
  Widget buildSegment(String text, int index, RoomMembersController controller) {
    return Container(
