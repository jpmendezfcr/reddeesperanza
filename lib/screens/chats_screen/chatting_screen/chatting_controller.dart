...
class ChattingController extends BlockUserController {
  ...
  DetectableTextEditingController messageTextController = DetectableTextEditingController(
    detectedStyle: MyTextStyle.gilroySemiBold(size: 16, color: cPrimary).copyWith(height: 1.2),
  ...
  ChattingController({this.room, this.user, ChatUserRoom? chatUserRoom, bool? shouldCallAPI}) {
    if (chatUserRoom != null) {
      ...
      this.chatUserRoom = ChatUserRoom(conversationId: room?.firebaseId(), title: room?.title ?? '', profileImage: room?.photo ?? '', userIdOrRoomId: room?.id, type: 2);
    } else if (user != null) {
      this.chatUserRoom = ChatUserRoom(conversationId: user?.id?.toConversationId(), title: user?.fullName ?? '', profileImage: user?.profile ?? '', userIdOrRoomId: user?.id, type: 1);
    }
    var firebaseUserIdentity = this.chatUserRoom?.userIdOrRoomId?.toString() ?? '';

    documentSender = db.collection(FirebaseConst.users).doc(myUser?.firebaseId()).collection(FirebaseConst.userList).doc(firebaseUserIdentity);

    documentReceiver = db.collection(FirebaseConst.users).doc(firebaseUserIdentity).collection(FirebaseConst.userList).doc(myUser?.firebaseId());
    //
    drChatMessages = db.collection(FirebaseConst.chats).doc(this.chatUserRoom?.conversationId ?? '').collection(FirebaseConst.messages);
  ...
  void commonSend({required MessageType type, String content = '', String thumbnail = '', num? storyId}) {
    ...
    var lastMsg = messageTextController.text.isEmpty ? (type == MessageType.image ? 'Image' : 'Video') : messageTextController.text;
    if (room != null) {
      var roomData = ChatUserRoom(conversationId: room?.firebaseId(), lastMsg: lastMsg, profileImage: room?.photo, title: room?.title, time: date, type: 2, usersIds: room?.roomUsers?.map((e) => e.id ?? 0).toList(), userIdOrRoomId: room?.id, unreadCounts: {}, deleteChatIds: {});
      chatUserRoom?.usersIds?.forEach((element) {
        roomData.unreadCounts?['${element.toInt()}'] = (chatUserRoom?.unreadCounts?['${element.toInt()}'] ?? 0) + 1;
        roomData.deleteChatIds?['${element.toInt()}'] = ((chatUserRoom?.deleteChatIds?['${element.toInt()}']) ?? '').replaceFirst(RegExp('d'), '');
      });
      ...
      NotificationService.shared.sendToTopic(topic: 'room_${room?.id ?? 0}', title: room?.title ?? '', body: '${myUser?.fullName ?? ''} : $lastMsg', conversationId: chatUserRoom?.conversationId ?? '');
    } else {
    ...
    if (user != null) {
      ...
      var myChatRoom = ChatUserRoom(isMute: false, profileImage: myUser?.profile, conversationId: chatUserRoom?.conversationId, lastMsg: lastMsg, title: myUser?.fullName, time: date, type: (status == 1) || (status == 3) ? 1 : 0, userIdOrRoomId: myUser?.id, newMsgCount: 1, isDeleted: false);
  ...
  void fetchDetailWithAPI() {
    if (chatUserRoom?.type != 2) {
      myUserListener = db.collection(FirebaseConst.users).doc("${chatUserRoom?.userIdOrRoomId?.toInt() ?? 0}").collection(FirebaseConst.userList).doc(myUser?.firebaseId()).snapshots().listen((event) {
        myChatRoom = ChatUserRoom.fromJson(event.data() ?? {});
        update();
      });
      userListener = db.collection(FirebaseConst.users).doc(myUser?.firebaseId()).collection(FirebaseConst.userList).doc("${chatUserRoom?.userIdOrRoomId?.toInt() ?? 0}").snapshots().listen((event) {
        if (event.data() != null) {
      ...
      this.room?.roomUsers = SessionManager.shared.getUsersForGroup(conversationId: chatUserRoom?.conversationId ?? '');
      ...
      RoomService.shared.fetchRoom(chatUserRoom?.userIdOrRoomId?.toInt() ?? 0, shouldShowMembers: true, (room) {
        this.room = room;
        SessionManager.shared.setUsersForGroup(conversationId: chatUserRoom?.conversationId ?? '', users: room.roomUsers ?? []);
        if (room.isMute == 0) {
  ...
  void deleteRoom() async {
    ...
    RoomService.shared.deleteRoom(room?.id ?? 0, () async {
      stopLoading();
      await db.collection(FirebaseConst.chats).doc(chatUserRoom?.conversationId).delete();
      Get.back();
    });
  }
